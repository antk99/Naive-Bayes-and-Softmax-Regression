...
def kfoldCV(x, y, model_str):
    ...
    for i in range(k):
        ...
        if model_str == "Naive":
            print(f'Running Naive Bayes using set {i} as validation set.')
            model = NaiveBayes()
            ...
            print("#########################################\n")
        elif model_str == "Softmax":
            print(f'Running Softmax using set {i}.\n')
    return
...
if __name__ == '__main__':
    ...
    if dataset == 1:    # 20 news groups dataset
        ...
        Sentiment140_test = pd.read_csv('./trainingandtestdata/testdata.manual.2009.06.14.csv', encoding='ISO-8859-1',
                                         header=None)
        ...
        Sentiment140 = pd.read_csv('./trainingandtestdata/training.1600000.processed.noemoticon.csv', encoding='ISO-8859-1',
                                         header=None).sample(5000)
        ...
        Sentiment140 = Sentiment140.sample(frac=1)
    ...
    model = NaiveBayes()
    print(f'Running Naive Bayes using the whole train set and test set.')
